...
import org.duvetmc.mods.rgmlquilt.RgmlLoadSystem;
...
//			readFromClassPath(source);
	...
	private static void initStats() {
		for(int id = 0; id < Block.BY_ID.length; ++id) {
			if (!Stats.BY_KEY.containsKey(16777216 + id) && Block.BY_ID[id] != null && Block.BY_ID[id].hasStats()) {
				...
				ItemStat stat = new ItemStat(16777216 + id, str, id);
		...
		for(int id = 0; id < Item.BY_ID.length; ++id) {
			if (!Stats.BY_KEY.containsKey(16908288 + id) && Item.BY_ID[id] != null) {
				...
				ItemStat stat = new ItemStat(16908288 + id, str, id);
	...
	public static void setupProperties(Class<? extends BaseMod> mod) throws IllegalArgumentException, IllegalAccessException, IOException, SecurityException, NoSuchFieldException {
